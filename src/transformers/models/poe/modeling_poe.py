...
class MixtralAttention(nn.Module):
    ...
    def __init__(self, config: PoEModelConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class MixtralSdpaAttention(MixtralAttention):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "PoEModel is using MixtralSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
class MixtralBlockSparseTop2MLP(nn.Module):
    def __init__(self, config: PoEModelConfig):
        super().__init__()
...
class SubPoEBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, final_hidden_states: torch.Tensor, expert_mask: torch.Tensor, hidden_dim: int, routing_weights: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx in range(self.num_experts):
            idx, top_x = torch.where(expert_mask[expert_idx + self.idx_shift])
...
class PreprocessBlock(nn.Module):
    def __init__(self, config: PoEModelConfig, layer_idx: int, sublayer_num: int):
        super().__init__()
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        # Self Attention
        hidden_states, self_attn_weights, present_key_value = self.self_attn[sublayer_idx](
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_value=past_key_value,
        ...
        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm[sublayer_idx](hidden_states)
...
class PoEDecoderLayer(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor,
        sublayer_idx: int, # [2024/10/23 Kuanting] Choose specific part to inference
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        past_key_value: Optional[Tuple[torch.Tensor]] = None,
        output_attentions: Optional[bool] = False,
...
class PoEPreTrainedModel(PreTrainedModel):
...
class PoEForCausalLM(PoEPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, PoEForCausalLM

        >>> model = PoEForCausalLM.from_pretrained("mistralai/Mixtral-8x7B-v0.1")
...
@add_start_docstrings(
    ...
    [`PoEForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
class PoEForSequenceClassification(PoEPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
